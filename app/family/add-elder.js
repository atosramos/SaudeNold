...
import { addFamilyElder } from '../../services/familyService';
...
export default function AddElder() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    birth_date: new Date(new Date().setFullYear(new Date().getFullYear() - 70)),
    gender: '',
    blood_type: '',
    email: '',
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addFamilyElder(payload);
      
      Alert.alert(
        'Sucesso',
        'Idoso adicionado à família com sucesso!',
        [
          {
            text: 'OK',
            onPress: () => router.back(),
      ...
      const errorMessage = error?.response?.data?.detail || error?.message || 'Erro ao adicionar idoso';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>Adicionar Idoso</Text>
      </View>
      ...
      <View style={styles.formContainer}>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.submitButtonText}>Adicionar Idoso</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>
