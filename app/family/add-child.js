...
import { addFamilyChild } from '../../services/familyService';
...
export default function AddChild() {
  ...
  const validateForm = () => {
    ...
    if (!formData.birth_date) {
      ...
      if (actualAge >= 18) {
        newErrors.birth_date = 'Criança deve ter menos de 18 anos';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addFamilyChild(payload);
      
      Alert.alert(
        'Sucesso',
        'Criança adicionada à família com sucesso!',
        [
          {
            text: 'OK',
            onPress: () => router.back(),
      ...
      const errorMessage = error?.response?.data?.detail || error?.message || 'Erro ao adicionar criança';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>Adicionar Criança</Text>
      </View>
      ...
      <View style={styles.formContainer}>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.submitButtonText}>Adicionar Criança</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>
